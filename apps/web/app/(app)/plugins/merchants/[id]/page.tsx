...
export default function MerchantPluginDetailPage() {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;

  const [plugin, setPlugin] = useState<MerchantPlugin | null>(null);
  ...
  const handleLaunch = async () => {
    ...
    setError(null);
    try {
      await api.post(`/merchant/launch/${id}`);
      await fetchPlugin();
    } catch (err: any) {
      setError(
  ...
  const handleDisable = async () => {
    ...
    setError(null);
    try {
      await api.post(`/merchant/disable/${id}`);
      await fetchPlugin();
    } catch (err: any) {
      setError(
  ...
  const handleUpdateConfig = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.put(`/merchant/config/${id}`, { config });
      await fetchPlugin();
    } catch (err: any) {
      setError(
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <Card>
        <div className="space-y-4">
          ...
          {plugin.allowCurrencyCodes &&
            plugin.allowCurrencyCodes.length > 0 && (
              <div>
                <span className="text-sm font-medium text-slate-700">
                  Allowed currencies
                </span>
                <div className="mt-1 flex flex-wrap gap-2">
                  {plugin.allowCurrencyCodes.map((code) => (
                    <span
                      key={code}
