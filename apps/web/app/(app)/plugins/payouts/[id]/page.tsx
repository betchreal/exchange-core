...
export default function PayoutPluginDetailPage() {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;

  const [plugin, setPlugin] = useState<PayoutPlugin | null>(null);
  ...
  const handleLaunch = async () => {
    ...
    setError(null);
    try {
      await api.post(`/payout/launch/${id}`);
      await fetchPlugin();
    } catch (err: any) {
      setError(
  ...
  const handleDisable = async () => {
    ...
    setError(null);
    try {
      await api.post(`/payout/disable/${id}`);
      await fetchPlugin();
    } catch (err: any) {
      setError(
  ...
  const handleUpdateConfig = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.put(`/payout/config/${id}`, { config });
      await fetchPlugin();
    } catch (err: any) {
      setError(
