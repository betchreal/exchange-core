...
export default function PayoutsPage() {
  const [plugins, setPlugins] = useState<PayoutPlugin[]>([]);
  ...
  const fetchPlugins = async () => {
    ...
    setError(null);
    try {
      const response = await api.get("/payout/all");
  ...
  if (loading) {
    return (
      <div className="flex w-full flex-col gap-6">
        <CardHeader
          title="Payout Plugins"
          description="Manage payout plugins for processing withdrawals"
        />
      ...
      {plugins.length === 0 ? (
        <Card>
          <div className="text-center text-sm text-slate-500">
            No payout plugins installed yet
          </div>
        </Card>
      ) : filteredPlugins.length === 0 ? (
        <Card>
          <div className="text-center text-sm text-slate-500">
            No payout plugins match "{search}"
          </div>
        </Card>
      ) : (
